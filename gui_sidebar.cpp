...
// Element symbols and display names shown in the sidebar list
static const std::vector<std::pair<std::string, std::string>> kElements = {
    {"H",  "H   Hydrogen   Val. 1"},
    {"O",  "O   Oxygen     Val. 2"},
    {"Na", "Na  Sodium     Val. 1"},
    {"Cl", "Cl  Chlorine   Val. 1"},
    {"Fe", "Fe  Iron       Val. 3"},
    {"C",  "C   Carbon     Val. 4"},
    {"N",  "N   Nitrogen   Val. 3"},
    {"S",  "S   Sulfur     Val. 2"},
...
void GuiSidebar::buildLayout() {
    ...
    for (const auto& [symbol, label] : kElements) {
        elementList_->Append(label);
    }
    sizer->Add(elementList_, 1, wxEXPAND | wxALL, 6);
    ...
    sizer->Add(historyList_, 1, wxEXPAND | wxALL, 6);
...
std::string GuiSidebar::getSelectedElement() const {
    ...
    return kElements[static_cast<size_t>(idx)].first;
}
